...
def solve(ranked, player):
    ...
    for s in rplayer:
        if s >= S:
            ...
            while ir < lenranked:
                S = ranked[ir]
                R += 1 if S < ranked[ir - 1] else 0
                ir += 1
                if s >= S:
                    foundinbetween = True
                    break
            rresult.append(R if foundinbetween else R + 1)
    return rresult[::-1]


actual = solve(ranked, player)
print(actual)
print(actual == expect)
